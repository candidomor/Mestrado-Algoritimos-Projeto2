...
def generate_unified_plot(scenarios, output_filepath):
    ...
    for name, data in scenarios.items():
        ...
        bst_mp = [d['bst_mp'] for d in data]
        avl_st = [d['avl_st'] for d in data]
        avl_mp = [d['avl_mp'] for d in data]

        # Linha contínua para BST, tracejada para AVL
        # Marcador O para Single-Thread (ST), S para Multi-Process (MP)
        
        plt.plot(N, bst_st, color=color, linestyle='-', marker='o', 
                 label=f'BST ST - {name}')
        plt.plot(N, bst_mp, color=color, linestyle='-', marker='s', 
                 label=f'BST MP - {name}', alpha=0.5)
                 
        plt.plot(N, avl_st, color=color, linestyle='--', marker='o', 
                 label=f'AVL ST - {name}')
        plt.plot(N, avl_mp, color=color, linestyle='--', marker='s', 
                 label=f'AVL MP - {name}', alpha=0.5)

    plt.title("V5 Multiprocessing Overhead - Todos os Cenários (BST e AVL)", fontsize=16)
    plt.xlabel("Volume de Dados (N)", fontsize=12)
    plt.ylabel("Tempo Total (Segundos) - Escala Log", fontsize=12)
    plt.yscale('log')
    
    # Configurando legenda na parte externa do gráfico para não sobrepor linhas
...
if __name__ == '__main__':
    ...
    output_file = 'v5_overhead_todos_cenarios_unificado.png'
    ...
    try:
        ...
        if not scenarios_data:
            ...
            generate_unified_plot(scenarios_data, output_file)
            print(f"Gráfico unificado gerado com sucesso em: {output_file}")
